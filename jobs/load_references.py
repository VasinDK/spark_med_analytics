...
def run_etl_reference():
    ...
    try:
        df_raw_departments = read_s3_csv(
            spark, build_s3_path(config["s3"]["departments_csv"]), schemas.departments
        )
        df_raw_professions = read_s3_csv(
            spark, build_s3_path(config["s3"]["professions_csv"]), schemas.professions
        )
        ...
        departments_table = (
            f"{config['db']['catalog']}."
            f"{config['db']['schema']}."
            f"{config['db']['table']['departments']}"
        )

        professions_table = (
            f"{config['db']['catalog']}."
            f"{config['db']['schema']}."
            f"{config['db']['table']['professions']}"
        )
