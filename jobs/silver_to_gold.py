...
def run_etl_gold(spark: SparkSession):
    ...
    df_visits = (
        spark.read.table(registry.get_table_address("silver", "visits"))
    ...
    df_active_visits = df_visits.select(
        col("id").alias("active_id"), col("visit_date").alias("active_date")
    ).distinct()

    df_symptoms_grouped = (
        spark.read.table(registry.get_table_address("silver", "visits_symptoms"))
        .join(
            df_active_visits,
            on=(
                (col("visit_id") == col("active_id"))
                & (col("visit_date") == col("active_date"))
            ),
            how="inner",
        )
    ...
    df_chronic_grouped = (
        spark.read.table(registry.get_table_address("silver", "visits_chronic"))
        .join(
            df_active_visits,
            on=(
                (col("visit_id") == col("active_id"))
    ...
    df_departments_prepared = spark.read.table(
        registry.get_table_address("silver", "departments")
    ).withColumnRenamed("name", "department_name")

    df_professions_prepared = spark.read.table(
        registry.get_table_address("silver", "professions")
    ...
    df_total = (
        ...
        .join(
            broadcast(df_departments_prepared),
            on=df_visits["department_id"] == df_departments_prepared["id"],
            how="left",
        )
        .drop(df_departments_prepared["id"])
        .join(
            broadcast(df_professions_prepared),
            on=df_visits["profession_id"] == df_professions_prepared["id"],
            how="left",
        )
