...
def generate_medical_dataset(output_filepath: str, num_records: int = 10):
    ...
    with open(output_filepath, "w", encoding="utf-8", buffering=64 * 1024) as f:
        ...
        for i in range(num_records):
            ...
            visit_date = (start_date + timedelta(days=random.randint(0, 130))).strftime(
                "%Y-%m-%d"
            )

            has_fever = random.random() < 0.15
            temperature = (
                round(random.uniform(38.0, 40.5), 1)
                if has_fever
                else round(random.uniform(36.2, 37.2), 1)
            )

            height = (
                random.randint(155, 195) if gender_id == 1 else random.randint(145, 180)
            )
            weight = (
                round(random.uniform(60.0, 110.0), 1)
                if gender_id == 1
                else round(random.uniform(45.0, 90.0), 1)
            )
    ...
    print(f"Successfully generated! File Size: {file_size_mb:.2f} MB")


if __name__ == "__main__":
    generate_medical_dataset(
        output_filepath="data/patient_visits_1m.json", num_records=10
    )
