...
def create_table_ice(
    spark: SparkSession, table_address: str, table_meta: Dict[str, Any]
):
...
def sync_table_columns(
    ...
    for col_name, _ in list(current_columns.items()):
        if col_name not in yaml_columns:
            logger.info(
                constants.DELETING_OUTDATED_COLUMN.format(col_name, target_address)
            )
    ...
    for field in yaml_fields:
        ...
        if f_name not in current_columns:
            ...
            logger.info(
                constants.ADDING_NEW_COLUMN.format(f_name, f_type_raw, target_address)
            )
            ...
            if current_type != type_raw:
                logger.info(
                    constants.CHANGING_COLUMN_TYPE.format(
                        f_name, target_address, current_type, f_type_raw
                    )
                )
                spark.sql(
                    f"ALTER TABLE {target_address} ALTER COLUMN {f_name} SET DATA TYPE {f_type_raw}"
                )
...
def getSparkType(type_raw: str):
    try:
        ...
        raise SyncTableError(
            constants.TYPE_COULD_NOT_BE_RECOGNIZED.format(type_raw)
        ) from e
