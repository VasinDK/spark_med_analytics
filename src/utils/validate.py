...
def validate(dq_config: dict):
    def _inner(df: DataFrame) -> DataFrame:
        age_condition_expr = "age < {} OR age > {}".format(
            dq_config["min_age"], dq_config["max_age"]
        )
        temp_condition_expr = "temperature < {} OR temperature > {}".format(
            dq_config["min_temp"], dq_config["max_temp"]
        )

        has_corrupt_col = "_corrupt_record" in df.columns
        corrupt_condition = (
            "_corrupt_record IS NOT NULL" if has_corrupt_col else "false"
        )
